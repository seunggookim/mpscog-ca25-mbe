...
connectivity_stg = connectivity.tocsr()[include, :].tocsc()[:, include]  # include should be vertno_ico5
##
dn_data = \
...
fn_pkl = dn_data + '/stg_X_NR_2x3_long.pkl'
if not os.path.isfile(fn_pkl):  # if not done yet:
    ...
    X_NR = list(range(2*3))
    n_sub = 10
    n_times = 661  # -0.2 to 0.5 sec (0.7 sec)
    n_vertices = len(include)
    
    for itype in range(2*3):
        X_NR[itype] = np.zeros([n_sub, n_times, n_vertices])
    ...
    # Pickle epoched responses:
    outfile = open(fn_pkl, 'wb')
    pkl.dump([X_NR, tstep, times, include,
              ['X_NR', 'tstep', 'times', 'include']],
             outfile)
    outfile.close()
    
    fn_mat = dn_data + '/stg_X_NR_2x3_long.mat'
    scipy.io.savemat(fn_mat, dict(X_NR=X_NR, times=times, include=include),
                     do_compression=True)
else:  # if done, read it!
    infile = open(fn_pkl, 'rb')
    [X_NR, tstep, times, include, _] = pkl.load(infile)
    infile.close()
...
n_times = 421
effects = ["A", "B", "A:B"]
effects_human = ['NR_Freq', 'NR_Type', 'NR_FreqxType']
factor_levels = [2, 3]
dn_out = dn_data
for effect, effect_human in zip(effects, effects_human):
    ...
    fn = dn_out + '/stg_2way_' + effect_human + '_k%d_long.pkl' % n_permutations
    if not os.path.isfile(fn):
        import time
        start_time = time.time()
        f, c, p, h0 = mne.stats.spatio_temporal_cluster_test(
            X_NR, stat_fun=stat_fun, threshold=f_thresh, tail=tail,
            n_jobs=20, n_permutations=n_permutations, buffer_size=None,
    ...
    p : (ndarray) #clusters
    '''
    fn_out = dn_out + "/stg_2way_" + effect_human + "_k%d_long.mat" % n_permutations
    scipy.io.savemat(
...
fn_pkl = dn_data + '/stg_X_RN_2x3_long.pkl'
if not os.path.isfile(fn_pkl):  # if not done yet:
    # READ source time course (STC) files of evoked responses:
    event_names = [
        'HC250NRN', 'HC250RNR', 'HC20NRN', 'HC20RNR',
    ...
    for itype in range(2 * 3):
        X_RN[itype] = np.zeros([n_sub, n_times, n_vertices])
    ...
    # Pickle epoched responses:
    outfile = open(fn_pkl, 'wb')
    pkl.dump([X_RN, tstep, times, include,
              ['X_RN', 'tstep', 'times', 'include']],
             outfile)
    outfile.close()
    
    fn_mat = dn_data + '/stg_X_RN_2x3_long.mat'
    scipy.io.savemat(fn_mat, dict(X_RN=X_RN, times=times, include=include),
                     do_compression=True)
else:  # if done, read it!
    infile = open(fn_pkl, 'rb')
    [X_RN, tstep, times, include, _] = pkl.load(infile)
    infile.close()
...
n_times = 661
effects = ["A", "B", "A:B"]
effects_human = ['RN_Freq', 'RN_Type', 'RN_FreqxType']
factor_levels = [2, 3]
dn_out = dn_data
for effect, effect_human in zip(effects, effects_human):
    # 1. Compute critical value for F-stat:
    ...
    fn = dn_out + '/stg_2way_' + effect_human + '_k%d.pkl' % n_permutations
    if not os.path.isfile(fn):
        import time
        
        start_time = time.time()
        f, c, p, h0 = mne.stats.spatio_temporal_cluster_test(
            X_RN, stat_fun=stat_fun, threshold=f_thresh, tail=tail,
            n_jobs=20, n_permutations=n_permutations, buffer_size=None,
            spatial_exclude=None, connectivity=connectivity_stg)
        outfile = open(fn, 'wb')
        pkl.dump([f, c, p, h0, ['f', 'c', 'p', 'h0']], outfile)
